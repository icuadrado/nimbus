...
public class DefaultDescribeSIAN implements DescribeSIAN {
    ...
    private static final Log logger =
            LogFactory.getLog(DefaultDescribeSIAN.class.getName());    
    ...
    public DefaultDescribeSIAN(Describe describeImpl, IDMappings idsImpl, ResourceAllocations RAsImpl) {
    ...
    public DescribeSpotInstanceRequestsResponseType describeSIRequests(
        ...
        SpotANRequestInfo[] spotRequests = null;
        ...
        if(sirisi != null) {
            ...
            try {
                if(reqIds.length > 0){
                    spotRequests = manager.getSpotANRequests(reqIds, caller);                
                } else {
                    spotRequests = manager.getSpotANRequestsByCaller(caller);
                }
            } catch (ManageException e) {
                final String msg = "Problem retrieving SIAN request: " + Arrays.toString(reqIds) + ": ";
    ...
    public SpotInstanceRequestSetItemType translateSpotInfo(
                                    SpotANRequestInfo result) throws Exception {
        ...
        sirsi.setType(type);
        //sirsi.setAdvanceNotice(""+result.getAdvanceNotice());
    ...
    protected LaunchSpecificationResponseType getLaunchSpec(SpotANRequestInfo reqInfo)
